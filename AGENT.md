...
├── index.html                   # inline anti-flash theme script in <head>
├── package.json                 # dev / build / lint / preview / worker:dev / worker:check / cf-typegen
├── tsconfig.json                # references app + node configs
├── tsconfig.app.json            # src/ — ES2023 + DOM
├── tsconfig.node.json
├── tsconfig.worker.json         # worker/ — ES2022, no DOM — Phase 1
├── wrangler.jsonc                # name/main/compatibility_date/vars/observability only — Phase 1
├── worker-configuration.d.ts     # generated by `wrangler types` — never hand-edit
├── vite.config.ts               # react + tailwind plugins, @ alias, server.proxy for /api and /ws
├── README.md                    # still the stock Vite template
├── public/{favicon.svg, icons.svg}
├── worker/
│   ├── index.ts                  # Hono app: CORS, timing middleware, onError, /api/health, /api/info
│   ├── env.ts                    # Env (ENVIRONMENT only), Variables (requestStart only)
│   └── lib/errors.ts             # ApiError
├── src/
│   ├── App.tsx                   # Navbar + CloudflareBar + placeholder main
│   ├── main.tsx
│   ├── index.css                # Tailwind v4 + the full oklch token set
│   ├── assets/
│   ├── lib/utils.ts             # re-exports cn — keep it, shadcn add depends on it
│   ├── api/client.ts             # apiFetch + subscribable X-DevBoard-* header store
│   └── components/
│       ├── ui/button.tsx        # the only shadcn primitive
│       └── layout/{Navbar,CloudflareBar}.tsx
└── docs/                        # these documents
```

### What does **not** exist yet

Any D1/R2/KV/Durable Object/Queue binding · any migration · any test · any test dependency · `.dev.vars` · `src/hooks/` · auth, projects, tasks, or any component beyond the Phase 1 shell.
...
| `npm run worker:dev` (`wrangler dev`) | Worker on :8787 — Phase 1 |
| `npm run worker:check` (`tsc -p tsconfig.worker.json --noEmit`) | Phase 1 |
| `npm run cf-typegen` (`wrangler types`) | Phase 1 |
